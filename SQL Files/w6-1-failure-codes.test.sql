-- Verification harness for w6-1-failure-codes.sql — OPTIONAL, safe.
-- Run AFTER the migration. Mutates NOTHING: wrapped in BEGIN ... ROLLBACK.
-- Read the NOTICEs: every non-SKIP line should say PASS.
-- Styled after b2-wo-categories.test.sql.
--
-- Proves:
--   1. an admin/manager of org A can INSERT a failure code into org A.
--   2. the same member CANNOT move that row into org B (UPDATE WITH CHECK).
--   3. the same member CANNOT INSERT a code for org B (INSERT WITH CHECK).
--   4. a technician CANNOT INSERT a code (role gate).
--
...
BEGIN
  ...
  INSERT INTO public.failure_codes (organisation_id, code, label)
  VALUES (v_org_a, 'HARNESS-1', 'Harness failure code')
  RETURNING id INTO v_fc;
  RAISE NOTICE 'PASS 1: admin/manager inserted a failure code into own org';

  -- 2) UPDATE that moves the row into org B must be rejected by WITH CHECK.
  v_ok := true;
  BEGIN
    UPDATE public.failure_codes SET organisation_id = v_org_b WHERE id = v_fc;
    v_ok := false;
  EXCEPTION WHEN insufficient_privilege THEN NULL; END;
  IF v_ok THEN RAISE NOTICE 'PASS 2: UPDATE org-swap into org B rejected';
  ELSE RAISE WARNING 'FAIL 2: UPDATE moved a failure code into another org'; END IF;

  -- 3) INSERT for org B must be rejected by INSERT WITH CHECK.
  v_ok := true;
  BEGIN
    INSERT INTO public.failure_codes (organisation_id, code, label)
    VALUES (v_org_b, 'HARNESS-2', 'Harness foreign code');
    v_ok := false;
  EXCEPTION WHEN insufficient_privilege THEN NULL; END;
  IF v_ok THEN RAISE NOTICE 'PASS 3: INSERT for foreign org rejected';
  ELSE RAISE WARNING 'FAIL 3: INSERT created a failure code for another org'; END IF;
  ...
  ELSE
    ...
    BEGIN
      INSERT INTO public.failure_codes (organisation_id, code, label)
      VALUES (v_org_a, 'HARNESS-3', 'Tech code');
      v_ok := false;
    EXCEPTION WHEN insufficient_privilege THEN NULL; END;
    IF v_ok THEN RAISE NOTICE 'PASS 4: technician INSERT rejected by role gate';
    ELSE RAISE WARNING 'FAIL 4: technician created a failure code'; END IF;
  END IF;
END $$;
ROLLBACK;
