...
export default function WorkOrderDetailScreen() {
  ...
  async function fetchWO() {
    ...
    // Load time logs from comments with type 'time_log'
  ...
  async function updateStatus(newStatus: string) {
    ...
    await supabase.from('work_order_comments').insert({
      ...
      user_id: profile?.id,
      comment: lang === 'ar'
        ? 'تم تغيير الحالة إلى: ' + statusLabels[newStatus]
  ...
  async function stopTimer() {
    ...
    const durationMins = Math.round((endTime.getTime() - startTimeRef.current.getTime()) / 60000)
    ...
    await supabase.from('work_order_comments').insert({
      ...
      user_id: profile?.id,
      comment: lang === 'ar'
        ? 'سجل وقت: ' + formatTime(durationSecs) + ' - بواسطة ' + profile?.full_name
        : 'Time logged: ' + formatTime(durationSecs) + ' by ' + profile?.full_name,
    ...
    // Update actual_hours on work order
    const currentHours = wo.actual_hours ?? 0
    const newHours = parseFloat((currentHours + durationMins / 60).toFixed(2))
    await supabase.from('work_orders').update({ actual_hours: newHours }).eq('id', wo.id)

    setTimerSeconds(0)
    startTimeRef.current = null
    await fetchWO()
    Alert.alert(
      lang === 'ar' ? 'تم تسجيل الوقت' : 'Time Logged',
      lang === 'ar' ? 'تم تسجيل ' + formatTime(durationSecs) : 'Logged ' + formatTime(durationSecs)
    )
  }
  ...
  async function pickAndUploadPhoto() {
    ...
    if (!permission.granted) {
      Alert.alert(lang === 'ar' ? 'مطلوب إذن الكاميرا' : 'Camera permission required')
      return
    }
    ...
    if (!error) {
      ...
      await fetchWO()
      Alert.alert(lang === 'ar' ? 'تم رفع الصورة' : 'Photo uploaded')
    } else {
  ...
  async function pickFromGallery() {
    ...
    if (!permission.granted) {
      Alert.alert(lang === 'ar' ? 'مطلوب إذن المعرض' : 'Gallery permission required')
      return
    }
  ...
  if (!wo) return (
    <View style={styles.centered}><Text>{lang === 'ar' ? 'أمر العمل غير موجود' : 'Work order not found'}</Text></View>
  )
  ...
  const statusActions: Record<string, { next: string; label: string; color: string }[]> = {
    ...
    in_progress: [
      { next: 'completed', label: t('complete'), color: colors.success },
  ...
  const allComments = comments.filter(c => !c.comment_type || c.comment_type === 'comment' || c.comment_type === 'status_change')

  const tabs = [
    { key: 'details',  label: lang === 'ar' ? 'التفاصيل' : 'Details' },
    { key: 'comments', label: lang === 'ar' ? 'التعليقات' : 'Comments', count: allComments.length },
    { key: 'photos',   label: lang === 'ar' ? 'الصور' : 'Photos', count: wo.photo_urls?.length ?? 0 },
        ...
        {activeTab === 'details' && (
          <View style={styles.card}>
            {[
              ...
              { label: t('due_date'),    value: wo.due_at ? format(new Date(wo.due_at), 'dd MMM yyyy HH:mm') : '-' },
              { label: lang === 'ar' ? 'بدأ في' : 'Started At', value: wo.started_at ? format(new Date(wo.started_at), 'dd MMM yyyy HH:mm') : '-' },
              { label: lang === 'ar' ? 'اكتمل في' : 'Completed At', value: wo.completed_at ? format(new Date(wo.completed_at), 'dd MMM yyyy HH:mm') : '-' },
              { label: lang === 'ar' ? 'ساعات العمل الفعلية' : 'Actual Hours', value: wo.actual_hours ? wo.actual_hours + ' hrs' : '-' },
        ...
        {activeTab === 'comments' && (
          <View style={styles.card}>
            {allComments.length === 0 ? (
              ...
              <View key={c.id} style={[styles.comment, c.comment_type === 'status_change' && styles.commentSystem]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={[styles.commentText, c.comment_type === 'status_change' && { color: colors.textSecondary, fontSize: 12 }]}>
        ...
        {activeTab === 'time' && (
          <View style={styles.card}>
            <View style={styles.timerBox}>
              ...
              <Text style={styles.timerLabel}>{timerRunning ? (lang === 'ar' ? 'جاري التسجيل...' : 'Recording...') : (lang === 'ar' ? 'مؤقت الوقت' : 'Time Tracker')}</Text>
            ...
            {wo.actual_hours && (
              <View style={[styles.row, { backgroundColor: colors.infoLight, borderRadius: 8, padding: 10 }]}>
                <Text style={styles.rowLabel}>{lang === 'ar' ? 'إجمالي ساعات العمل' : 'Total Hours Worked'}</Text>
                <Text style={[styles.rowValue, { color: colors.info, fontWeight: '700' }]}>{wo.actual_hours} hrs</Text>
              </View>
            )}
            {timeLogs.length === 0 ? (
              <Text style={styles.empty}>{lang === 'ar' ? 'لا توجد سجلات وقت بعد' : 'No time logs yet'}</Text>
            ) : timeLogs.map(log => (
...
const styles = StyleSheet.create({
  ...
  titleBadges: { flexDirection: 'row', gap: 8, marginBottom: 8 },
  ...
  commentSystem: { backgroundColor: colors.background, borderRadius: 8, padding: 8 },
  ...
  timerDisplay: { fontSize: 48, fontWeight: '700', color: colors.primary, fontVariant: ['tabular-nums'] },
...
with open('src/screens/WorkOrderDetailScreen.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print('WorkOrderDetailScreen completely rewritten with all features')

# Also add comment_type column to work_order_comments if needed
print('NOTE: Run this SQL in Supabase if comment_type column does not exist:')
print("ALTER TABLE work_order_comments ADD COLUMN IF NOT EXISTS comment_type text DEFAULT 'comment';")
print("ALTER TABLE work_order_comments ADD COLUMN IF NOT EXISTS metadata jsonb;")
print("ALTER TABLE work_orders ADD COLUMN IF NOT EXISTS actual_hours decimal(10,2);")
print("ALTER TABLE work_orders ADD COLUMN IF NOT EXISTS started_at timestamptz;")