    ...
    <div style={{ padding: '2rem', maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {templates.map(t => (
            <button
              key={t.label}
              type='button'
              onClick={() => setForm(prev => ({ ...prev, title: t.label, description: t.description }))}
              style={{ padding: '5px 12px', borderRadius: 20, border: '1px solid #ddd', background: 'white', cursor: 'pointer', fontSize: 12, color: '#444' }}
            >
              {t.label}
            </button>
          ))}
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        <div style={{ background: '#f0f7ff', border: '1px solid #b3d4f5', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#1565c0' }}>
          When this schedule is due, click Generate WO on the schedules list to create a work order automatically. The next due date will be calculated automatically after each generation.
        </div>
