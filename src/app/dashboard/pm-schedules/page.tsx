...
export default function PMSchedulesPage() {
  ...
  const isDue = (schedule: any) => schedule.next_due_at && isPast(new Date(schedule.next_due_at))
  const isDueSoon = (schedule: any) => {
    if (!schedule.next_due_at) return false
    const days = Math.ceil((new Date(schedule.next_due_at).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
  ...
  const stats = {
    ...
    dueSoon: schedules.filter(s => s.is_active && isDueSoon(s) && !isDue(s)).length,
  }
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#999', margin: '4px 0 0' }}>
            ...
            {stats.dueSoon > 0 && <span style={{ color: '#f57f17' }}> · {stats.dueSoon} due this week</span>}
          </p>
        </div>
                  ...
                  <tr key={s.id} style={{ borderBottom: '1px solid #f0f0f0', background: due && s.is_active ? '#fff8f8' : i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        {s.is_active && (
                          <button
                            onClick={() => generateWorkOrder(s)}
                            disabled={generating === s.id}
                            style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid #1a1a2e', background: 'white', color: '#1a1a2e', cursor: 'pointer', fontSize: 12, fontWeight: 500 }}
                          >
                        ...
                        <button
                          ...
                          style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid #ddd', background: 'white', color: '#666', cursor: 'pointer', fontSize: 12 }}
                        >
