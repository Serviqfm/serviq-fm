...
export default function AssetDetailPage() {
  ...
  async function fetchWorkOrders() {
    const { data } = await supabase.from('work_orders').select('*, assignee:assigned_to(full_name)').eq('asset_id', id).order('created_at', { ascending: false })
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ marginTop: '1rem', marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#999', fontSize: 13, marginTop: 6 }}>Added {format(new Date(asset.created_at), 'dd MMM yyyy')} · {workOrders.length} work orders total · {openWOs} open</p>
      </div>
