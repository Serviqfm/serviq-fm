...
export default function InventoryPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#999', margin: '4px 0 0' }}>
            ...
            {lowStockItems.length > 0 && <span style={{ color: '#c62828', marginLeft: 8 }}>· {lowStockItems.length} {t('inv.status.low')}</span>}
          </p>
        </div>
        <Link href='/dashboard/inventory/new'>
          <button style={{ background: '#1a1a2e', color: 'white', padding: '8px 20px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 500 }}>{t('btn.add_item')}</button>
        </Link>
      </div>

      {lowStockItems.length > 0 && (
        <div style={{ background: '#fce4ec', border: '1px solid #ef9a9a', borderRadius: 10, padding: '12px 16px', marginBottom: '1.5rem' }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: '#b71c1c', margin: '0 0 6px' }}>{t('inv.low_stock')}</p>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('inv.search')} style={{ width: '100%', padding: '9px 14px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, marginBottom: '1rem', boxSizing: 'border-box' as const }} />
      ...
      {selected.length > 0 && (
        <div style={{ background: '#fce4ec', border: '1px solid #ef9a9a', borderRadius: 10, padding: '10px 16px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: '#b71c1c' }}>{selected.length} {t('common.selected') || 'selected'}</span>
                  ...
                  <tr key={item.id} style={{ borderBottom: '1px solid #f0f0f0', background: selected.includes(item.id) ? '#f3f4fd' : isLow ? '#fff8f8' : i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button onClick={() => deleteOne(item.id)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ef9a9a', background: '#fce4ec', color: '#c62828', cursor: 'pointer', fontSize: 11 }}>{t('common.delete')}</button>
                      </div>
                    </td>
                  </tr>
                )
