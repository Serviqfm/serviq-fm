...
function qrSize(layout: number) {
  return layout === 2 ? 140 : layout === 4 ? 90 : 60
}
function nameSize(layout: number) {
  return layout === 2 ? 13 : layout === 4 ? 9 : 7
}
function subSize(layout: number) {
  return layout === 2 ? 10 : layout === 4 ? 7 : 6
}
        ...
        <View style={styles.grid}>
          {qrImages.map((item, i) => (
            <View key={i} style={cardStyle(layout)}>
              ...
              <Text style={{ fontSize: sbSz, color: '#64748B', textAlign: 'center' }}>{item.floor}</Text>
              <Text style={{ fontSize: sbSz, color: '#64748B', textAlign: 'center' }}>{siteName}</Text>
            </View>
          ))}
        </View>
      </Page>
