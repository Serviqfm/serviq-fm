...
// Generic files tab: upload a file (or attach an existing org file) to any
// entity via the polymorphic `file_attachments` table. Extracted from the
// original WorkOrderFilesTab so sites/assets/etc. reuse the exact same flow.
...
export default function EntityFilesTab({
  entityType, entityId, orgId, bucket = 'work-order-media',
}: { entityType: string; entityId: string; orgId: string; bucket?: string }) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const params = new URLSearchParams({ bucket, prefix: `${orgId}/${entityType}/${entityId}` })
    ...
    await supabase.from('file_attachments').insert({ organisation_id: orgId, file_id: fileRow.id, entity_type: entityType, entity_id: entityId })
    setUploading(false)
    await load()
  }

  async function attachExisting(fileId: string) {
    await supabase.from('file_attachments').insert({ organisation_id: orgId, file_id: fileId, entity_type: entityType, entity_id: entityId })
    setShowAttach(false)
    await load()
  }

  async function detach(attachmentId: string) {
    if (!confirm(lang === 'ar' ? 'إزالة هذا الملف؟ (يبقى في مكتبة الملفات)' : 'Remove this file? (it stays in the Files library)')) return
