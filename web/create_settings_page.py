...
export default function SettingsPage() {
  ...
  async function fetchData() {
    ...
    const { data: profile } = await supabase.from('users').select('*, organisation:organisation_id(*)').eq('id', authUser.id).single()
    if (profile) {
      setUser(profile)
        ...
        <div>
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <label style={labelStyle}>{lang === 'ar' ? 'اسم المؤسسة (إنجليزي) *' : 'Organisation Name (English) *'}</label>
                <input style={inputStyle} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder='e.g. Al Noor School' />
              </div>
              <div>
                <label style={labelStyle}>{lang === 'ar' ? 'اسم المؤسسة (عربي)' : 'Organisation Name (Arabic)'}</label>
              ...
              <div>
                <label style={labelStyle}>{lang === 'ar' ? 'الرقم الضريبي (VAT)' : 'VAT Number'}</label>
                <input style={inputStyle} value={form.vat_number} onChange={e => setForm(f => ({ ...f, vat_number: e.target.value }))} placeholder='e.g. 300000000000003' />
              </div>
              <div>
                <label style={labelStyle}>{lang === 'ar' ? 'رقم السجل التجاري' : 'Commercial Registration (CR)'}</label>
                <input style={inputStyle} value={form.cr_number} onChange={e => setForm(f => ({ ...f, cr_number: e.target.value }))} placeholder='e.g. 1010000000' />
              </div>
              <div>
              ...
              <div>
                ...
                <input style={inputStyle} value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} placeholder='e.g. Riyadh' />
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={labelStyle}>{lang === 'ar' ? 'العنوان' : 'Address'}</label>
                <input style={inputStyle} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder='e.g. King Fahd Road, Al Olaya District' />
              </div>
              <div>
            ...
            <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={saveOrg} disabled={saving} style={{ background: '#1a1a2e', color: 'white', padding: '10px 28px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 500, fontSize: 14 }}>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f9f9f9', borderRadius: 10, padding: '1rem 1.25rem' }}>
              <div>
                <span style={{ background: planInfo.color, color: 'white', padding: '3px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600 }}>{planInfo.label}</span>
                <p style={{ fontSize: 13, color: '#666', margin: '8px 0 0' }}>{planInfo.price}</p>
              </div>
              <div style={{ textAlign: 'right' as const }}>
                <p style={{ fontSize: 12, color: '#999', margin: 0 }}>{lang === 'ar' ? 'للترقية تواصل معنا' : 'To upgrade, contact us'}</p>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ background: '#e3f2fd', borderRadius: 10, padding: '1rem 1.25rem', marginBottom: '1.25rem' }}>
              ...
              <p style={{ fontSize: 13, color: '#1565c0', margin: 0 }}>
                {lang === 'ar'
                  ? 'يتم حذف الصور ومقاطع الفيديو المرفقة بأوامر العمل والتفتيش تلقائياً بعد 6 أشهر من تاريخ الرفع.'
                  : 'Images and videos attached to work orders and inspections are automatically purged 6 months after upload.'}
              </p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: '1.5rem' }}>
              {[
                { icon: '✅', title: lang === 'ar' ? 'البيانات الهيكلية' : 'Structured Data', desc: lang === 'ar' ? 'أوامر العمل، الأصول، سجلات الصيانة — محفوظة بشكل دائم على جميع الخطط' : 'Work orders, assets, PM logs — retained permanently on all plans' },
                { icon: '📷', title: lang === 'ar' ? 'الصور والفيديو' : 'Images & Video', desc: lang === 'ar' ? 'تُحذف تلقائياً بعد 6 أشهر. يمكنك تحميلها في أي وقت قبل الحذف.' : 'Auto-purged after 6 months. Export at any time before purge date.' },
                { icon: '🔔', title: lang === 'ar' ? 'إشعار مسبق' : 'Advance Notice', desc: lang === 'ar' ? 'تصلك رسالة بريد إلكتروني قبل 30 يوماً من حذف أي ملفات وسائط.' : 'You receive an email 30 days before any media files are purged.' },
                { icon: '📦', title: lang === 'ar' ? 'تصدير البيانات' : 'Data Export', desc: lang === 'ar' ? 'قم بتحميل جميع ملفاتك الوسائطية في أي وقت من لوحة التحكم.' : 'Download all your media files at any time from the dashboard.' },
              ].map(item => (
            ...
            {[
              { period: lang === 'ar' ? '12 شهراً' : '12 months total', price: 'SAR 600/yr', best: lang === 'ar' ? 'مناسب للمدارس' : 'Best for Schools' },
              { period: lang === 'ar' ? '24 شهراً' : '24 months total', price: 'SAR 1,200/yr', best: lang === 'ar' ? 'مناسب للتجزئة' : 'Best for Retail' },
              { period: lang === 'ar' ? 'غير محدود' : 'Unlimited',       price: 'SAR 2,400/yr', best: lang === 'ar' ? 'مناسب للفنادق والمجمعات' : 'Best for Hotels & Compounds' },
            ].map(addon => (
              <div key={addon.price} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', border: '1px solid #eee', borderRadius: 10, marginBottom: 8 }}>
                ...
                <div style={{ textAlign: 'right' as const }}>
                  <p style={{ fontSize: 14, fontWeight: 600, color: '#1a1a2e', margin: '0 0 2px' }}>{addon.price}</p>
                  <a href='mailto:support@serviqfm.com' style={{ fontSize: 12, color: '#1a1a2e' }}>{lang === 'ar' ? 'تواصل للاشتراك' : 'Contact to subscribe'}</a>
                </div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {(['ar', 'en'] as const).map(l => (
                <button key={l} onClick={() => setLang(l)}
                  style={{ padding: '10px 24px', borderRadius: 8, border: lang === l ? 'none' : '1px solid #ddd', background: lang === l ? '#1a1a2e' : 'white', color: lang === l ? 'white' : '#333', cursor: 'pointer', fontSize: 14, fontWeight: lang === l ? 600 : 400 }}>
