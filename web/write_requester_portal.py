...
os.makedirs('src/app/request', exist_ok=True)
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: 'white', borderRadius: 16, padding: '2.5rem', maxWidth: 480, width: '100%', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        <button
          onClick={() => { setSuccess(false); setForm({ title: '', description: '', location: '', site_id: '', priority: 'medium', requester_name: profile?.full_name ?? '', requester_phone: '' }) }}
...
with open('src/app/request/page.tsx', 'w', encoding='utf-8') as f:
    f.write(portal)
print('Requester portal written successfully')